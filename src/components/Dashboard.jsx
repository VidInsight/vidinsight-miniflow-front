...
import { Search, Filter, Workflow, TrendingUp, Clock, AlertCircle, RefreshCw } from 'lucide-react';
...
function Dashboard() {
  ...
  const { workflows, loading, error, refreshWorkflows, createNewWorkflow } = useWorkflows();
  ...
  const handleNewWorkflow = async () => {
    try {
      ...
      const newWorkflow = await createNewWorkflow({
        ...
        description: 'Yeni oluşturulan workflow - düzenlemeye hazır'
      });
      
      // Yeni workflow oluşturulduktan sonra workflow builder'a yönlendir
      navigate(`/workflow-builder/${newWorkflow.id}`);
    } catch (error) {
      console.error('Failed to create new workflow:', error);
      // Hata durumunda bile workflow builder'a git (offline mod)
          ...
          <div className="flex justify-between items-center py-6">
            ...
            <div className="flex items-center space-x-4">
              ...
              <button 
                ...
                disabled={isCreatingWorkflow || loading}
                className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2"
              >
                {isCreatingWorkflow && (
                  <RefreshCw className="w-4 h-4 animate-spin" />
                )}
                <span>{isCreatingWorkflow ? 'Oluşturuluyor...' : 'New Workflow'}</span>
              </button>
            </div>
          </div>
        </div>
